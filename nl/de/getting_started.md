...
Dieses Lernprogramm verwendet die Beispielapp [compose-mysql-helloworld-nodejs](https://github.com/IBM-Cloud/compose-mysql-helloworld-nodejs), um zu veranschaulichen, wie eine Verbindung zu einem {{site.data.keyword.composeForMySQL_full}}-Service hergestellt wird. Die Anwendung erstellt eine Datenbank, liest daraus und schreibt darin. Dabei nutzt sie Daten, die über die Webschnittstelle der App bereitgestellt werden.
{: shortdesc}
...
Dieser Schritt schlägt fehl, wenn der Service die Bereitstellung aus Schritt 1 noch nicht abgeschlossen hat. Sie können den Fortschritt der Bereitstellung überprüfen, indem Sie für den Service in die Ansicht _Verwalten_ wechseln.
{: .tip}
...
## Schritt 8: Prüfen, ob die App mit Ihrem {{site.data.keyword.composeForMySQL}}-Service verbunden ist
...
6. Erstellen Sie in Ihrem bevorzugten Editor eine neue Datei mit folgenden Angaben. Ersetzen Sie dabei den Text 'BERECHTIGUNGSNACHWEISE HIER EINFÜGEN' mit dem gesamten Inhalt der Berechtigungsnachweise, die Sie im vorherigen Schritt kopiert haben:
