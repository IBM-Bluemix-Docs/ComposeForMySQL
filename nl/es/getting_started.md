...
Esta guía de aprendizaje utiliza la app de ejemplo [compose-mysql-helloworld-nodejs](https://github.com/IBM-Cloud/compose-mysql-helloworld-nodejs) para demostrar cómo conectar una aplicación a un servicio {{site.data.keyword.composeForMySQL_full}}. La aplicación crea, lee y escribe en una base de datos utilizando datos suministrados a través de la interfaz web de la app.
{: shortdesc}
...
## Paso 1: Crear una instancia de servicio de {{site.data.keyword.composeForMySQL}}
{: #create-service}
...
4. Actualice el valor `services` para que coincida con el nombre del servicio que ha creado en [Crear una instancia de servicio de {{site.data.keyword.composeForMySQL}}](#create-service).

## Paso 7: Enviar la app a {{site.data.keyword.cloud_notm}}.

Este paso fallará si el servicio no ha finalizado el suministro desde el Paso 1. Puede comprobar el progreso de suministro para asegurarse de que se ha completado al ir a la vista _Gestionar_ para el servicio.
{: .tip}
...
## Paso 8: Comprobar que la app está conectada al servicio de {{site.data.keyword.composeForMySQL}}

1. Vaya al panel de control de servicio de {{site.data.keyword.composeForMySQL}}
...
Ahora, cuando se visite `<host>.mybluemix.net/`, podrá ver el contenido de la recopilación de {{site.data.keyword.composeForMySQL}}. A medida que añada palabras y sus definiciones, se añadirán a la base de datos y se mostrarán. Si detiene y reinicia la app, verá que aparece listada cualquier palabra y definición que ya haya añadido.


## Ejecución de la app localmente

En lugar de enviar la app en {{site.data.keyword.cloud_notm}}, puede ejecutarla localmente para probar la conexión a su instancia de servicio de {{site.data.keyword.composeForMySQL}}. Para conectarse al servicio deberá crear un conjunto de credenciales de servicio.

1. Desde el panel de control de {{site.data.keyword.cloud_notm}}, abra la instancia de servicio de {{site.data.keyword.composeForMySQL}}.
...
6. En el editor de su elección, cree un nuevo archivo con lo siguiente, sustituyendo el texto 'INSERT YOUR CREDENTIALS HERE' con el contenido completo de las credenciales que ha copiado en el paso anterior:
...
La app se está ejecutando en [http://localhost:8080](http://localhost:8080). Puede añadir palabras y definiciones a la base de datos de {{site.data.keyword.composeForMySQL}}. Cuando se detenga y se reinicie la app, cualquier palabra que haya añadido ya se mostrará al renovar la página.
...
Para obtener más información sobre cómo funciona la app de ejemplo [compose-mysql-helloworld-nodejs](https://github.com/IBM-Cloud/compose-mysql-helloworld-nodejs), puede leer el archivo readme de la aplicación, o los comentarios de código en `server.js`, que dan alguna información sobre las funciones de la app.

Para empezar a explorar su servicio de {{site.data.keyword.composeForMySQL}}, consulte los temas siguientes sobre el panel de control del servicio:
