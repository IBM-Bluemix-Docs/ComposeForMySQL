...
Procédez comme suit pour restaurer une sauvegarde d'un service MySQL en cours d'exécution vers un nouveau service MySQL à l'aide de l'interface de ligne de commande {{site.data.keyword.cloud_notm}}. 
...
```
  _SERVICE_ doit avoir pour valeur compose-for-mysql et _PLAN_ doit avoir pour valeur Standard ou Enterprise, en fonction de votre environnement. Pour _SERVICE\_INSTANCE\_NAME_, indiquez le nom de votre nouveau service. _source\_service\_instance\_id_ correspond à l'ID d'instance de service de la source de la sauvegarde ; pour l'obtenir, exécutez `ibmcloud cf service DISPLAY_NAME --guid` où _DISPLAY\_NAME_ est le nom du service MySQL à l'origine de la sauvegarde. 
...
Par exemple, la restauration d'une version plus ancienne d'un service {{site.data.keyword.composeForMySQL}} vers un nouveau service exécutant MySQL 5.7.20 est similaire à l'exemple suivant :
```
