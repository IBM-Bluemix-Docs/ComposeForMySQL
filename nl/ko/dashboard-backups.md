...
![백업](./images/mysql-backups-show.png "사용 가능한 백업 목록")

해당 행을 클릭하여 사용 가능한 백업에 대한 옵션을 펼치십시오.

![백업 옵션](./images/mysql-backups-options.png "백업에 대한 옵션") 
...
백업을 새 서비스 인스턴스에 복원하려면 단계에 따라 기존 백업을 확인한 후 해당 행을 클릭하여 다운로드할 백업에 대한 옵션을 펼치십시오. **복원** 단추를 클릭하십시오. 복원이 시작되었음을 알리는 메시지가 표시됩니다. 새 서비스 인스턴스가 자동으로 "mysql-restore-[timestamp]"로 이름 지정되고 프로비저닝이 시작될 때 대시보드에 표시됩니다.

### {{site.data.keyword.cloud_notm}} CLI를 통해 복원

{{site.data.keyword.cloud_notm}} CLI를 사용하여 실행 중인 MySQL 서비스의 백업을 새 MySQL 서비스에 복원하려면 다음 단계를 사용하십시오. 
...
``` 
  ...
  ```
  _SERVICE_ 필드는 compose-for-mysql이어야 하며 _PLAN_ 필드는 사용자 환경에 따라 Standard 또는 Enterprise여야 합니다. _SERVICE\_INSTANCE\_NAME_은 새 서비스의 이름을 배치할 위치입니다. _source\_service\_instance\_id_는 백업 소스의 서비스 인스턴스 ID이며 `ibmcloud cf service DISPLAY_NAME --guid`를 실행하여 얻을 수 있습니다. 여기서 _DISPLAY\_NAME_은 백업이 생성된 MySQL 서비스의 이름입니다. 
...
예를 들어, 이전 버전의 {{site.data.keyword.composeForMySQL}} 서비스를 MySQL 5.7.20이 실행되는 새 서비스로 복원하는 방법은 다음과 같습니다.
```
