...
# 入门教程
本教程使用 [compose-mysql-helloworld-nodejs](https://github.com/IBM-Cloud/compose-mysql-helloworld-nodejs) 样本应用程序来演示如何将应用程序连接到 {{site.data.keyword.composeForMySQL_full}} 服务。应用程序可使用通过应用程序的 Web 界面提供的数据来对数据库执行创建、读取和写入操作。
{: shortdesc}
...
## 步骤 1：创建 {{site.data.keyword.composeForMySQL}} 服务实例
{: #create-service}
...
接下来，选择服务的价格套餐。可以选择*标准*或*企业*套餐。使用*企业*套餐，您可以将 {{site.data.keyword.composeForMySQL}} 实例供应到可用的 {{site.data.keyword.composeEnterprise}} 集群中。{{site.data.keyword.composeEnterprise}} 提供企业合规性所需的安全性和隔离，并使用专用网络来确保已部署的数据库的性能。有关更多详细信息，请参阅 [{{site.data.keyword.composeEnterprise}}](/docs/services/ComposeEnterprise/index.html) 文档。
...
4. 更新 `services` 值，以匹配在[创建 {{site.data.keyword.composeForMySQL}} 服务实例](#create-service)中所创建服务的名称。

## 步骤 7：将应用程序推送到 {{site.data.keyword.cloud_notm}}。

如果服务尚未完成步骤 1 中的供应，那么此步骤会失败。您可以通过转至服务的_管理_视图来检查供应进度，以确保该步骤完成。
{: .tip}
...
## 步骤 8：检查应用程序是否连接到 {{site.data.keyword.composeForMySQL}} 服务

1. 浏览至 {{site.data.keyword.composeForMySQL}} 服务仪表板。
...
现在，访问 `<host>.mybluemix.net/` 时，您将能够查看 {{site.data.keyword.composeForMySQL}} 集合的内容。添加字词及其定义时，这些内容将添加到数据库并显示。如果停止并重新启动应用程序，您将看到已添加的所有字词和定义现在均已列出。


## 本地运行应用程序

可以不将应用程序推送到 {{site.data.keyword.cloud_notm}}，而改为在本地运行应用程序，以测试它与 {{site.data.keyword.composeForMySQL}} 服务实例的连接。要连接到该服务，您需要创建一组服务凭证。

1. 在 {{site.data.keyword.cloud_notm}}“仪表板”中，打开 {{site.data.keyword.composeForMySQL}} 服务实例。
2. 从主菜单中选择_服务凭证_以打开“服务凭证”视图。
3. 单击**新建凭证**。
4. 选择凭证的名称，然后单击**添加**。
5. 现在将列出您的新凭证。单击表中相应行中的**查看凭证**以查看凭证，然后单击**复制**图标以复制凭证。
6. 在您选择的编辑器中，创建包含以下内容的新文件，然后将文本“INSERT YOUR CREDENTIALS HERE”替换为上一步中复制的凭证的完整内容：
...
现在，应用程序会在 [http://localhost:8080](http://localhost:8080) 上运行。您可以向 {{site.data.keyword.composeForMySQL}} 数据库添加字词和定义。停止并重新启动应用程序时，刷新页面后会显示已添加的所有字词。
...
要了解有关 [compose-mysql-helloworld-nodejs](https://github.com/IBM-Cloud/compose-mysql-helloworld-nodejs) 样本应用程序工作方式的更多信息，可以阅读应用程序自述文件或阅读 `server.js` 中的代码注释，其中提供了有关应用程序功能的一些信息。

要开始探索 {{site.data.keyword.composeForMySQL}} 服务，请查看有关服务仪表板的以下主题：
